import { initializeApp, getApps } from 'firebase/app';
import { getAuth, GoogleAuthProvider, signInWithPopup, signOut } from 'firebase/auth';
import { getFirestore, doc, getDoc, setDoc, collection, onSnapshot, query, deleteDoc } from 'firebase/firestore';

// Placeholder for firebase-applet-config.json which is generated during provisioning
// Users should replace this with their actual config from the Firebase Console
let firebaseConfig = {
  apiKey: "PLACEHOLDER",
  authDomain: "PLACEHOLDER",
  projectId: "PLACEHOLDER",
  storageBucket: "PLACEHOLDER",
  messagingSenderId: "PLACEHOLDER",
  appId: "PLACEHOLDER"
};

// @ts-ignore - This file is created dynamically during build/provisioning
import config from '../../firebase-applet-config.json' assert { type: 'json' };
if (config) {
  firebaseConfig = config as any;
}

const app = getApps().length === 0 ? initializeApp(firebaseConfig) : getApps()[0];
export const auth = getAuth(app);
export const db = getFirestore(app);
export const googleProvider = new GoogleAuthProvider();

// Auth helpers
export const loginWithGoogle = () => signInWithPopup(auth, googleProvider);
export const logout = () => signOut(auth);

// Firestore helpers
export { doc, getDoc, setDoc, collection, onSnapshot, query, deleteDoc };
